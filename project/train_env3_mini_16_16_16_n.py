...
1. Size: 
   Environments: 16 * 16 * 16
   Matched .binvox model: 8 * 8 * 6
2. Movement: 
...
class working_space_train3(gym.Env):
    '''  
    Funcitons:
        -------------------------
    ...
    def __init__(self,
                 material_name = 'train_material3_8_8_6.binvox',
                 product_name = 'train_product3_8_8_6_n.binvox',
                 space_x = 16, 
                 space_y = 16, 
                 space_z = 16,
                 s_neutral = 0,
...
#----------- From here all the same for each training environment ------------#
    ...
    def sensor(self, position):
        ...
        for sight_position in sight_positions:
            ...
            if x_s in range(self.space_x) and y_s in range(self.space_y) and z_s in range(self.space_z):
                s_value = self.sight_space[x_s][y_s][z_s]
            else:
                s_value = self.sight_option['out']
    ...
    def step(self, action):
        '''
        action: [move]
        state: [s_down, s_up, s_left0, s_right0, s_back0, s_front0, 
                scan1, scan2, scan3, scan4, scan5, scan6, scan7, scan8]
        '''
        ...
        return self.state, step_reward, self.effective_num, self.false_num, \
               mission_complete, mission_incomplete, out, end, self.moves
    ...
    def reset(self):
        ...
        Reset variables:
            ...
            sight_space        'np.ndarray'
            reward_space       'np.ndarray'
            position           'np.ndarray'
            start              'np.ndarray'
            state              'np.ndarray'
            false_num          'int'
